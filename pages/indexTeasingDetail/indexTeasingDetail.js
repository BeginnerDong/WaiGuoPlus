...
Page({
	...
	getAnonymousData() {
		...
		const callback = (res) => {
			...
			var num = Math.floor(Math.random() * self.data.anonymousData.length);
		...
		const callback = (res) => {
			if (res.info.data.length > 0) {
				...
				/* if(self.data.mainData.length>2){
				   self.data.mainData = self.data.mainData.splice(0,2)
				 }   */
			} else {
	...
	showMore(e) {
		...
		if (self.data.mainData[index].answerData) {
			...
			wx.navigateTo({
				url: "/pages/indexTeasingComment/indexTeasingComment?id=" + self.data.mainData[index].id,
			})
		} else {
